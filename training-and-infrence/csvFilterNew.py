...
class SerialReaderApp:
    ...
    def is_csv(self, data):
        """Check if a line of data is in CSV format."""
        return data.count(",") > 0
    ...
    def read_serial(self):
        while not self.stop_reading:
            if self.serial_port.in_waiting > 0:
                ...
                if self.is_csv(data):
                    self.save_csv_line(data)
                    
            time.sleep(0.01)

    def save_csv_line(self, data):
        ...
        file_path = os.path.join("raw_rec", f"{file_name}{self.count}.csv")

        with open(file_path, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(data.split(","))
        print(f"Saved CSV data: {data}")
